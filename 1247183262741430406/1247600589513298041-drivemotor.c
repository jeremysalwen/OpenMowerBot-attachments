...
static uint32_t bump_started = 0; //Used to enforce minimum delay on bump sensor
static uint8_t bump_count = 0; //Stores the number of bumps seen in this session.
    ...
    switch (drivemotor_eState)
    {
        ...
        case DRIVEMOTOR_RUN:
            ...
            if((HALLSTOP_Left_Sense() || HALLSTOP_Right_Sense()) \
                ...
                if (bump_started==0)
                {
                    bump_started=now; //Start bump timer as not currently running
                }
                switch (main_eOpenmowerStatus){
                    case OPENMOWER_STATUS_MOWING:
                        if (now - bump_started >= BUMP_MILLIS_WHILE_MOWING){
                            ...
                            if (bump_count > MAX_BUMP_COUNT_PER_MOW) //If too many bumps - go home
                            {
                                buttonupdated = 1; //Simulate Home button push
                                buttoncleared = 0; //Simulate Home button release
                                buttonstate[PANEL_BUTTON_DEF_HOME]= 1;
                                bump_count = 0;
                            }
                         }
			        break;
                    case OPENMOWER_STATUS_DOCKING:
                        ...
                        bump_count = 0; //Reset the bump count
                ...
                bump_started = 0; //Reset bump counter
            }
