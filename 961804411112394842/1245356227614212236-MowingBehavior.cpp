...
bool MowingBehavior::create_mowing_plan(int area_index) {
    ...
    if(mowingPlanDigest == currentMowingPlanDigest) {
        ...
        if(currentMowingPaths[currentMowingPath].is_outline && !getConfig().add_fake_obstacle &&
                            (currentMowingPaths[currentMowingPath].outline_start_index < currentMowingPathIndex) &&
                            (currentMowingPaths[currentMowingPath].outline_end_index > currentMowingPathIndex)) {
            currentMowingPathIndex = currentMowingPaths[currentMowingPath].outline_start_index;
            if(currentMowingPathIndex > 10)
                currentMowingPathIndex -= 10; //if possible back up by around 1m into previous outline so we have the transition move
            ROS_INFO_STREAM("MowingBehavior: On perimeter, resetting path to point: " << currentMowingPathIndex);
        }
    } else {
        ROS_INFO_STREAM(
...
bool MowingBehavior::execute_mowing_plan() {
    ...
    bool add_fake_obstacle = getConfig().add_fake_obstacle;
    ...
    while (currentMowingPath < currentMowingPaths.size() && ros::ok() && !aborted) {
        ...
        /////////////////////////////////////////////////////////////////////////////////////////////////////////
        {
            ...
            if(currentMowingPaths[currentMowingPath].is_outline && add_fake_obstacle &&
                            (currentMowingPaths[currentMowingPath].outline_start_index < currentMowingPathIndex) &&
                            (currentMowingPaths[currentMowingPath].outline_end_index > currentMowingPathIndex)) {
            ...
            while (ros::ok()) {
                ...
                if (current_status.state_ == actionlib::SimpleClientGoalState::ACTIVE ||
                    ...
                    if(skip_path) {
                        skip_path= false;
                        currentMowingPath++;
                        add_fake_obstacle = getConfig().add_fake_obstacle;
                        currentMowingPathIndex = 0;
                        return false;
                    }
                    if (aborted) {
            ...
            if (current_status.state_ != actionlib::SimpleClientGoalState::SUCCEEDED) {
                ...
                else
                {
                    ...
                    {
                        // Unable to reach the start of the mow path (we tried multiple attempts for the same point, and we skipped points which also didnt work
                        if(currentMowingPaths[currentMowingPath].is_outline && add_fake_obstacle &&
                                    (currentMowingPaths[currentMowingPath].outline_start_index < currentMowingPathIndex) &&
                                    (currentMowingPaths[currentMowingPath].outline_end_index > currentMowingPathIndex))  { //if haven't tried starting from beginning then do so
                            add_fake_obstacle = false;
                            currentMowingPathIndex = currentMowingPaths[currentMowingPath].outline_start_index;
                            if(currentMowingPathIndex > 10)
                                currentMowingPathIndex -= 10; //if possible back up by around 1m into previous outline so we have the transition move
                            ROS_INFO_STREAM("MowingBehavior: On perimeter, resetting path to start point: " << currentMowingPathIndex);
                        }
