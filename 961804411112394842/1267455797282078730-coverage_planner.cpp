    ...
    for (int i = 0; i < group.size(); i++) {
        ...
        if (path.path.poses.empty()) {
            ...
            double min_start_distance = INFINITY;
                ...
                for (int start_search = 0; start_search < poly.points.size(); ++start_search) {
                    const auto &pt = poly.points[start_search];
                ...
                line = poly.split_at_index(min_start_index);
        } else {
        ...
        if(isObstacle) {
            if(i==0) { //if at end of inner loop
        ...
        if(isObstacle) {
            ...
            else if(i==1) { //multiple loops so include transition path
                path.outline_start_index++;
                ROS_INFO_STREAM("Obstruction outline starts at point " << path.outline_start_index);
            }
